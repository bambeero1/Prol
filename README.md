...
```bash
docker run -d -p 80:80 -p 8081:8081 -v /var/www/scylla:/var/www/scylla --name scylla wildcat/scylla:latest
```
...
```bash
http://localhost:80/api/v1/proxies
```
...
```bash
http://localhost:80/api/v1/stats
```
...
Open `http://localhost:80` in your browser to see the Web UI of this
project.
